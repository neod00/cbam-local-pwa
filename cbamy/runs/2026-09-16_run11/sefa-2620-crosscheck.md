...
1. ~~영문 원문 대조~~ — 완료(위 표).
2. ~~**3.3(2)(a)~(d)의 B열 선택** — 전구물질의 원산지·생산경로·합금 등급에 따라 B열 값이 갈리는 CN이 있다(같은 CN에 값이 둘 이상이면 5.1은 "가장 높은 값"). 앱은 CN과 생산경로 문자열로만 찾는다.~~ 2026-09-19 수정. 5.3의 표시 문자((1)(2) 생산연도, (C)~(J) 경로·등급)로 고른다. 원산국 기본 경로는 DV 워크북의 경로 열에서 읽고, 남으면 5.1대로 최고값, 근거가 없으면 화면에 경고한다.
...
4. **전구물질의 보고기간(2025/2547 제13조)** — (1)·(2) 시기 구분에 따라 벤치마크가 달라지는 품목이 있다. 2026-09-19: (1)·(2)는 가정의 기본값 연도로 고른다. 전구물질마다 다른 생산연도를 넣는 입력은 없다.
