...
- 2026-09-11: Allocation logic follows Implementing Regulation (EU) 2025/2547 verbatim anchors (rule IDs `CBAM-ALLOC-*` in `src/lib/allocation-rules.ts`): activity level per Annex II point F (off-spec/by-product/waste/scrap lines are excluded from the SEE denominator and carry zero emissions once the user marks them; legacy lines keep their numbers and only raise a confirmation warning), reconciliation factor per Annex III A.1 Eq. 41-42 for sub-metered shared meters only (allocation-key splits get a sum check, not a factor), direct-emission input mode recorded per process (source-stream sum / manual / template upload), manual allocation treated as a regulation exception (Annex III A.2 functional-unit rule) that requires a reason and a 100 % sum, and one production process per installation/period/CN (Article 4(6)). Heat, waste-gas and on-site power adjustments (Annex III A.3 Eq. 55) remain unsupported and are labelled as such; they are detected, not computed.
