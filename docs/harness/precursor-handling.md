...
| 다수 공급사 믹스 (④) | 실무 | 백로그(우회 O) | 파일럿이 다공급사 입력을 **불편해할 때**. 그전엔 같은 CN을 **여러 행으로 입력 → 엔진이 정확 가중합산** |
| 간접 SEE 2입력 분리(bridge) | E_PurchPrec (e) 실구조 | 백로그 | **제3자 검증·제출 준비 시** 또는 **비철강(간접값 있는) 전구물질** 등장 시. 철강 전구물질은 간접≈0이라 거의 미발동 |
| SAD 비교 (③) | KB §2 | 백로그(부분 O) | 사용자가 actual↔default **비용 최적화**를 원할 때(일부 /scenarios가 커버) |
| 공급사 요청 5종 필드(생산경로·보고기간) | Guidance Step 2 | 백로그 | 실제 공급사 통신·제출 단계 진입 시 |
...
> ⑤·⑦ 완료. 나머지(③④⑥·bridge·5종필드)는 파일럿 피드백으로 트리거되면 착수.
