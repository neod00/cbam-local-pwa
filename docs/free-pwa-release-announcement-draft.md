...
- 국영문 DOCX 산정근거 요약 보고서와 증빙 체크리스트를 포함한 전달 패키지 생성
...
전달 패키지에는 수정 가능한 국영문 DOCX 산정근거 요약 보고서와 증빙 체크리스트를 함께 넣을 수 있습니다. 단, 패키지 안의 `.cbam` 백업에는 민감한 로컬 입력자료가 포함될 수 있으므로 회사 승인 없이 수입자, 컨설턴트, 검증기관에 공유하지 마세요.
