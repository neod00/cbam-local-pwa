...
// 시범 디자인(getdesign.md · Stripe 계열): 이 페이지에만 Inter 얇은 웨이트를 로드해 스코프한다.
const stripeFont = Inter({ subsets: ['latin'], weight: ['300', '400', '500', '600'], display: 'swap' });

// Stripe DESIGN.md에서 뽑은 토큰(딥네이비 잉크·인디고 액센트·헤어라인·부드러운 네이비 그림자).
const S = {
    ink: '#0d253d',
    inkSec: '#273951',
    inkMute: '#64748d',
...
const SHADOW_1 = '0 1px 3px rgba(0,55,112,0.08)';
const SHADOW_2 = '0 8px 24px rgba(0,55,112,0.10), 0 2px 6px rgba(0,55,112,0.05)';
const TNUM = { fontFeatureSettings: "'tnum'" } as const;

// 시범 디자인 전용 패널(얇은 제목 + 헤어라인 카드). SectionCard는 드문 경고 섹션에만 남겨 둔다.
function Panel({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    return (
        <section className="rounded-2xl p-5 sm:p-6" style={{ backgroundColor: S.canvas, border: `1px solid ${S.hairline}`, boxShadow: SHADOW_1 }}>
            <h2 className="text-[17px] font-light tracking-[-0.01em]" style={{ color: S.ink }}>{title}</h2>
            {subtitle && <p className="mt-1 text-[13px] font-light leading-6" style={{ color: S.inkMute }}>{subtitle}</p>}
...
function CollapsibleSection({ title, summary, defaultOpen = false, children }: {
    ...
    return (
        <section className="rounded-2xl" style={{ backgroundColor: S.canvas, border: `1px solid ${S.hairline}`, boxShadow: SHADOW_1 }}>
            <button
                type="button"
            ...
            >
                <span className="min-w-0">
                    <span className="text-[15px] font-normal tracking-[-0.01em]" style={{ color: S.ink }}>{title}</span>
                    {summary && <span className="ml-2 text-[12px] font-light" style={{ color: S.inkMute }}>{summary}</span>}
                </span>
                <ChevronDown className={`h-4 w-4 flex-none transition ${open ? 'rotate-180' : ''}`} style={{ color: S.inkMute }} />
            </button>
            {open && <div className="px-5 py-5" style={{ borderTop: `1px solid ${S.hairline}` }}>{children}</div>}
        </section>
    );
}
...
export default function ScenariosPage() {
    ...
    return (
        <div className={`${stripeFont.className} space-y-6`} style={{ fontFeatureSettings: "'ss01'", color: S.ink }}>
            <div>
                <p className="text-[11px] font-medium uppercase tracking-[0.14em]" style={{ color: S.primary }}>고급 검토</p>
                <h1 className="mt-1.5 text-[30px] font-light leading-[1.1] tracking-[-0.02em]" style={{ color: S.ink }}>인증서 비용 시나리오</h1>
                <p className="mt-2 max-w-2xl text-[15px] font-light leading-[1.6]" style={{ color: S.inkMute }}>
            ...
            <section className="overflow-hidden rounded-2xl p-6 sm:p-7" style={{ backgroundColor: S.navy, boxShadow: SHADOW_2 }}>
                <div className="flex flex-wrap items-center gap-3">
                    <h2 className="text-[20px] font-light tracking-[-0.01em] text-white">이번 시나리오 결론</h2>
                    <span
                        ...
                        style={
                            summary.totalImportMass === 0
                                ...
                                : isDeMinimisCandidate
                                    ? { backgroundColor: 'rgba(142,227,196,0.16)', color: S.onNavyBright }
                                    : { backgroundColor: 'rgba(243,199,120,0.16)', color: S.onNavyWarm }
                        }
                    >
                        {summary.totalImportMass === 0
                            ? '수입량 입력 필요'
                ...
                <p className="mt-2.5 max-w-3xl text-[14px] font-light leading-[1.65]" style={{ color: S.onNavy }}>
                ...
                <div className="mt-5 grid grid-cols-1 gap-px overflow-hidden rounded-xl sm:grid-cols-3" style={{ backgroundColor: 'rgba(255,255,255,0.08)' }}>
                    <div className="p-4" style={{ backgroundColor: S.navy }}>
                        ...
                        <p className="mt-2 text-[26px] font-light leading-none tracking-[-0.02em] text-white" style={TNUM}>
                            {loading ? '—' : formatInteger(summary.totalCertificateQuantity)} <span className="text-[13px] font-light" style={{ color: S.onNavy }}>tCO₂e</span>
                        </p>
                        <p className="mt-2 text-[12px] font-light leading-5" style={{ color: S.onNavy }}>
                    ...
                    <div className="p-4" style={{ backgroundColor: S.navy }}>
                        ...
                        <p className="mt-2 text-[26px] font-light leading-none tracking-[-0.02em] text-white" style={TNUM}>{loading ? '—' : formatCurrency(summary.totalCost)}</p>
                        <p className="mt-2 text-[12px] font-light leading-5" style={{ color: S.onNavy }}>
                    ...
                    {!hasDefaultComparison ? (
                        <div className="p-4" style={{ backgroundColor: S.navy }}>
                            ...
                            <p className="mt-2 text-[26px] font-light leading-none tracking-[-0.02em] text-white">비교 불가</p>
                            <p className="mt-2 text-[12px] font-light leading-5" style={{ color: S.onNavy }}>기준자료(기본값 파일)를 연결하면 유리한 쪽을 판정합니다.</p>
                        </div>
                    ) : costDelta > 0.5 ? (
                        <div className="p-4" style={{ backgroundColor: S.navy }}>
                            <p className="text-[11px] font-medium uppercase tracking-[0.08em]" style={{ color: S.onNavyWarm }}>기본값이 유리</p>
                            <p className="mt-2 text-[26px] font-light leading-none tracking-[-0.02em]" style={{ color: S.onNavyWarm, ...TNUM }}>−{formatCurrency(costDelta)}</p>
                            <p className="mt-2 text-[12px] font-light leading-5" style={{ color: S.onNavy }}>
                        ...
                        <div className="p-4" style={{ backgroundColor: S.navy }}>
                            ...
                            <p className="mt-2 text-[26px] font-light leading-none tracking-[-0.02em]" style={{ color: S.onNavyBright, ...TNUM }}>−{formatCurrency(Math.abs(costDelta))}</p>
                            <p className="mt-2 text-[12px] font-light leading-5" style={{ color: S.onNavy }}>기본값({formatCurrency(summary.totalDefaultCost)})보다 낮습니다 — 실측 자료를 유지·보강하세요.</p>
                        </div>
                    ) : (
                        <div className="p-4" style={{ backgroundColor: S.navy }}>
                            <p className="text-[11px] font-medium uppercase tracking-[0.08em]" style={{ color: S.onNavy }}>실측 vs 기본값</p>
                            <p className="mt-2 text-[26px] font-light leading-none tracking-[-0.02em] text-white">차이 거의 없음</p>
                            <p className="mt-2 text-[12px] font-light leading-5" style={{ color: S.onNavy }}>증빙이 쉬운 쪽을 선택하세요.</p>
                        </div>
                    )}
                </div>
            </section>
            ...
            >
                {scenarios.length === 0 ? (
                    <div className="rounded-xl p-5 text-center text-[13px] font-light" style={{ border: `1px dashed ${S.hairline}`, color: S.inkMute }}>
                            ...
                            return (
                                <div key={`${scenario.result_id}-verdict`} className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl px-4 py-3.5" style={{ border: `1px solid ${S.hairline}`, backgroundColor: S.canvas }}>
                                    <div className="min-w-40">
                                        ...
                                        <p className="mt-0.5 text-[12px] font-light" style={{ color: S.inkMute, ...TNUM }}>
                                            {scenario.cn_code ? `CN ${scenario.cn_code}` : 'CN 미입력'} · EU {formatInteger(scenario.import_mass_t)}t
                                        </p>
                                    </div>
                                    <p className="text-[13px] font-light" style={{ color: S.inkSec, ...TNUM }}>
                                    ...
                                    <div className="ml-auto text-right">
                                        {isDefaultBetter && delta !== undefined ? (
                                            <>
                                                ...
                                                <p className="text-[12px] font-light" style={{ color: S.inkMute }}>기본값 근거 기록 후 사용 검토</p>
                                            </>
                                            ...
                                            <>
                                                ...
                                                <p className="text-[12px] font-light" style={{ color: S.inkMute }}>공급사 실측 유지 · 검증 대비</p>
                                            </>
                                        ) : scenario.lower_certificate_basis === 'TIE' ? (
                                            <p className="text-[12px] font-light" style={{ color: S.inkMute }}>실측·기본값 차이 없음 — 증빙 쉬운 쪽 선택</p>
                                        ) : (
                                            <p className="text-[12px] font-light" style={{ color: S.inkMute }}>기준자료 연결 후 판정됩니다</p>
                                        )}
                                    </div>
                                </div>
                            );
                            ...
                            <div key={item.key} className="rounded-xl p-4" style={{ border: `1px solid ${S.hairline}`, backgroundColor: S.canvasSoft }}>
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 text-[13px] font-light leading-6" style={{ color: S.inkMute }}>{item.description}</p>
                                        {item.href && item.cta && (
